...
- **Win+D immunity / smart-topmost** — `App` holds a `SetWinEventHook(EVENT_SYSTEM_FOREGROUND)`: when the foreground is Progman/WorkerW (desktop) frames go `HWND_TOPMOST`, otherwise not (plus `LowerBelowForeground` so they don't cover the just-activated window). **Critical gotcha: Win32 implicitly assigns an OWNER to top-level windows; an owned window's `WS_EX_TOPMOST` is constrained by its owner and the kernel SILENTLY strips the flag (SetWindowPos/SetWindowLong both "succeed", err=0, style reads back unchanged).** Fix: `NonActivatingWindow.OnSourceInitialized` clears the owner via `SetWindowLongPtr(GWLP_HWNDPARENT, NULL)`. Frames also set `ShowActivated = false` (a (re)shown frame must never take foreground — the foreground hook would read it as "user switched to an app" and un-topmost everything; the hook additionally ignores our own frame HWNDs). `SetAllTopmost` verifies each flag post-set and `ReopenFrame`s any HWND that still refuses. WorkerW/Progman reparenting is a dead end on this machine — see `docs/workerw-z-order-analysis.md`.
...
- ✅ Win+D immunity (smart-topmost: WinEvent foreground hook → frames float when desktop is foreground; 220ms re-assert; per-HWND verify + reopen self-heal)
